...
const widgetEntries = glob.sync('./src/widgets/**/*.tsx').reduce((obj, el) => {
  ...
  const filepath = './' + path.relative(__dirname, el).replace(/\\/g, '/');
  obj[rel] = filepath;
  obj[`${rel}${SANDBOX_SUFFIX}`] = filepath;
  return obj;
}, {});

const validWidgetNames = Object.keys(widgetEntries).filter((name) => !name.endsWith(SANDBOX_SUFFIX));
