...
const entryMap = glob.sync('./src/widgets/**/*.tsx').reduce((obj, el) => {
  const rel = path
...
const validWidgetNames = Object.keys(entryMap).filter((k) => !k.endsWith(SANDBOX_SUFFIX));
    ...
    headers: (req, res, context) => {
      ...
      try {
        ...
        if (
          ...
          (originUrl && (originUrl.hostname === 'localhost' || originUrl.hostname === '127.0.0.1'))
        ) {
