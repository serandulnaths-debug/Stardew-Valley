...
const entryPoints = glob.sync('./src/widgets/**/*.tsx').reduce((obj, el) => {
  const rel = path
    .relative(path.join(__dirname, 'src/widgets'), path.join(__dirname, el))
    .replace(/\.[tj]sx?$/, '')
    .replace(/\\/g, '/');

  // Prefix file paths with './' relative to the directory where webpack is run
  const relativeFilePath = './' + path.relative(__dirname, el).replace(/\\/g, '/');

  obj[rel] = relativeFilePath;
  obj[`${rel}${SANDBOX_SUFFIX}`] = relativeFilePath;
  return obj;
}, {});

// Extract base widget names without the sandbox suffix
const validWidgetNames = Object.keys(entryPoints).filter(name => !name.endsWith(SANDBOX_SUFFIX));
