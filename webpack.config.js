...
const validWidgetNames = Object.keys(entryPoints).filter((k) => !k.endsWith(SANDBOX_SUFFIX));
    ...
    headers: (req, _res, _context) => {
      ...
      try {
        if (origin) {
          ...
          isLocalhost = originUrl.hostname === 'localhost' || originUrl.hostname === '127.0.0.1';
        }
      } catch (_e) {
        // Invalid origin URL
      }
