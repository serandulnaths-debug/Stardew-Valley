...
const widgetEntries = glob.sync('./src/widgets/**/*.tsx').reduce((obj, el) => {
  ...
  const fileRelPath = './' + path.relative(__dirname, el).replace(/\\/g, '/');
  obj[rel] = fileRelPath;
  obj[`${rel}${SANDBOX_SUFFIX}`] = fileRelPath;
  return obj;
}, {});

const allowedWidgetNames = Object.keys(widgetEntries).filter(name => !name.endsWith(SANDBOX_SUFFIX));
