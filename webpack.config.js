...
const entryPoints = glob.sync('./src/widgets/**/*.tsx').reduce((obj, el) => {
  ...
  // Webpack 5 requires relative paths for entries within the current directory
  // if they aren't meant to resolve to node_modules.
...
// Valid widget names are the entry points without the sandbox suffix
const validWidgetNames = Object.keys(entryPoints).filter(
  (name) => !name.endsWith(SANDBOX_SUFFIX)
);
